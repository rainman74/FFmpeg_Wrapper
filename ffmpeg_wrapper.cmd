...
call :SETESC
call :SETTOKEN
set "FF_FLAGS=-v info -hide_banner -stats -err_detect ignore_err -fflags +genpts+igndts -hwaccel auto"
    ...
    if not defined SKIP_FILE (
        ...
        if "!REQ_Q!"=="auto" (
            echo "!FILENAME!" | findstr /c:"(19" >nul || echo "!FILENAME!" | findstr /c:"(20" >nul || (
                echo %ESC%[91mWARNING: No year found → fallback quality (!QUALITY!^)%ESC%[0m
            )
        )
        ...
        if not defined SKIP_FILE (
            ffmpeg %FF_FLAGS% !DECODER_PARAM! -i "%%I" -map 0 ^
                -c:v %ENCODER% -profile:v %PROFILE% -level auto ^
                -rc vbr -cq:v !QUALITY! !PRESET! -multipass fullres ^
                -spatial_aq 1 -temporal_aq 1 -aq-strength 10 ^
                -rc-lookahead 24 !TUNING! !B_REF! ^
                !VF_PARAM! !AUDIO! ^
                -c:s copy -map_metadata 0 -map_chapters 0 ^
                "_Converted\%%~nI.mkv"
...
if "%6"=="tweak"      (set "MODE=eq=contrast=1.05:brightness=0.02:saturation=1.1")
...
cls
echo Usage: %~n0 ^<encoder^> [audio] [quality] [crop/resize] [filter] [mode] [decoder]
echo.
call :PRINT_TOK "encoder" "(erforderlich)"  TOK_ENCODER
...
echo.
echo crop/resize: none / auto / 696..1800 (Letterbox entfernen) oder 720,720p,1080,1080p,2160,... (Skalierung auf diese Höhe)
echo.
echo Beispiele:
echo   %~n0 hevc copy def auto none HDRtoSDR cuda
echo   %~n0 hevc copy hq 1080p deblock yadif auto
echo   %~n0 h264 copy uhq 800 sharp deint sw
echo   %~n0 av1 copy def none nlmeans brighter cuda
endlocal
goto :END
...
set "TOK_FILTER=none text reverb deblock edgelevel smooth smooth31 smooth63 nlmeans gauss gauss5 sharp ss denoise denoisehq artifact artifacthq superres superreshq vsr vsrdenoise vsrdenoisehq vsrartifact vsrartifacthq log f1 f2 f3 f4 f5 f6"
